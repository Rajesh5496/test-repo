...
class Finalize
{
    ...
    public function service($request)
    {
        ...
        $result->debugs[] = "Start finalize on target REST service with max time: " . $timer->getRemainingSyncTime();
        ...
        foreach ($delete as $key => &$deletepath) {
            ...
            if (!file_exists($filepath)) {
                $result->debugs[] = "Finalize remote REST service: Could not find file/dir that is on delete array, so ignoring the file: " . $filepath;
            ...
            if (is_file($filepath)) {
                ...
                $result->debugs[] = "Finalize remote REST service: Starting deleting: " . $filepath;
                if ($delete_result === false) {
                    // Delete did not complete within timeframe
                    $result->debugs[] = "Finalize remote REST service: Could not complete delete within max time for: " . $filepath;
                } else {
                    $deleted = true;
                }
            }
            if ($deleted) {
                $result->debugs[] = "Finalize remote REST service: Deleted " . $filepath;
                $deletepath->deleted = true;
            }

            if (!$timer->shouldContinueWithLastrunTime(3)) {
                $result->debugs[] = "Finalize remote REST service file/dir needs to abort due to max execution time";
                break;
            }
        }

        // When all is deleted, we have completed
        $result->debugs[] = "Finalize remote REST service file/dir deleted completed";
