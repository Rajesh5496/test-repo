...
class PopulateFileList
{
    ...
    public function handleRunning()
    {
        ...
        // Run until we have around one second left
        while ($this->timer->getRemainingSyncTime() > 2) {
            ...
            foreach ($filter_iterator as $fileinfo) {
                ...
                // check every 2 seconds, if we need to break out because of time or large filelist
                if ((microtime(true) - $check_timeout_timer) > 1) {
                    if ($this->timer->getRemainingSyncTime() < 2) {
                        $is_completed = false;
                        break;
                    }
                    $check_timeout_timer = microtime(true);
     ...
     * @since 1.4.0
     */
    public function resetFilePopulationTable()
    {
        global $wpdb;
