...
class DatabaseSync
{
     ...
     * @since 1.0.0
     */
    public function getBaseSearchReplaces()
    {
        $searchreplaces = array();

        // Domain search replace
        $sr1 = new \stdClass();
        $sr1->from = untrailingslashit($this->job->from_client_home_url);
        $sr1->to = untrailingslashit($this->job->to_client_home_url);
        $searchreplaces[] = $sr1;

        // Path search replace
        $sr2 = new \stdClass();
        $sr2->from = $this->job->from_files_home_dir;
        $sr2->to = $this->job->to_files_home_dir;
        $searchreplaces[] = $sr2;
    ...
    private function prepareSyncData()
    {
        ...
        $this->searchreplaces = $this->getBaseSearchReplaces();
        $this->searchreplaces = array_merge($this->installation->searchreplaces, $this->searchreplaces);
    ...
    public function callRemoteClientDBService(&$body, $to_or_from = 'to')
    {
        ...
        if ($database_result->isSuccess()) {
            ...
            $this->job->errors[] = __("Remote/local site responded with error, which means we can not continue the synchronization. Check log for details and contact support if this persist.", "wpsynchro");
        }
    }
