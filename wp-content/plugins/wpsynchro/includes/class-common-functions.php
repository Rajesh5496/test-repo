    ...
    public function validateTransferToken($token_to_validate)
    {
        ...
        if (is_object($current_transfer)) {
            ...
            if ($current_transfer->last_activity > (time() - $current_transfer->lifetime)) {
                // Still existing transfer, so check if IP matches
                ...
                $current_ip_list = $common->getClientIPAddress();
                $all_matched = true;
                foreach ($current_ip_list as $checkip) {
                    if (!in_array($checkip, $current_transfer->clientip)) {
                        $all_matched = false;
                    }
                }

                // If all matched, its okay
                if ($all_matched) {
                    ...
                    // The IP client list does not match the one at initiation               
                    return false;
                }
            } else {
                // Too old                   
