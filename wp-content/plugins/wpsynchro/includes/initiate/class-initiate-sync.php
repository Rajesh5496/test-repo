...
class InitiateSync
{
    ...
    public function retrieveTransferTokenFromURL($url)
    {
        ...
        if ($initiate_result->isSuccess()) {
            ...
            $this->job->errors[] = __("Failed initialization - Did not get a response from remote server - Check logs", "wpsynchro");
            $this->logger->log("DEBUG", "Failed initializing - Did not get a response from remote server");
        }
        return "";
    }
}
