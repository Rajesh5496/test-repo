    ...
    public function runMasterdataStep(&$installation, &$job)
    {
        ...
        foreach ($metadata_results as $prefix => $masterdata) {
            if (!$masterdata->isSuccess()) {
                $errormsg = sprintf(__("Could not retrieve masterdata from target '%s' - See log file", "wpsynchro"), $prefix);
        ...
        foreach ($metadata_results as $prefix => $masterdata) {
            ...
            foreach ($mappings as $job_key => $masterdata_key) {
                if (!isset($masterdata_content->$masterdata_key)) {
                    continue;
                }
                $tmp_var = $prefix . $job_key;
                $this->job->$tmp_var = $masterdata_content->$masterdata_key;
            }
        }
